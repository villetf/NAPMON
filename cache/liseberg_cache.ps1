...
foreach ($attNum in $attList) {
    ...
    try {
        #Invoke-Expression -Command "mysql -u villetf -pkevsuger liseberg -h 192.168.1.13 -e 'INSERT INTO attraction_metrics (a_id, time, queue_time, closing_time) VALUES ($attNum, DATE_FORMAT(NOW(), ''%Y-%m-%d %H:%i:00''), $idResultTime, $idResultClose);UPDATE attractions SET status = ''$($idResult.status)'' WHERE a_id = $attNum'"
        Add-Content ./liseberg.csv "`"$attNum`",`"$((Get-Date).AddHours(-2).ToString("yyyy-MM-dd HH:mm:ss"))`",`"$idResultTime`",`"$idResultClose`""
    }
    catch {
