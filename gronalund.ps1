...
foreach ($attraction in $queueList) {
   if ($attraction.attractionId -in $attList) {
      ...
      try {
         Invoke-Expression -Command "mysql --skip-ssl -u villetf -pkevsuger gronalund -h 192.168.1.13 -e 'INSERT INTO attraction_metrics (a_id, time, queue_time) VALUES ($attId, DATE_FORMAT(NOW(), ''%Y-%m-%d %H:%i:00''), $queueTime);UPDATE attractions SET status = $queueTime WHERE a_id = $attId'"
      }
      catch {
